    ...
    <>
      {pagesArr.map(function (pageNumber) {
        let css = pageNumber == cPage ?
          "hover:bg-blue-500  border-2 py-2 px-3 rounded bg-blue-600 text-white" :
          "hover:bg-red-450  border-2 py-2 px-3 rounded ";
        return (<button className={css} key={pageNumber} style={{width:"55px"}}
          onClick={() => {
            setcPage(pageNumber)
          }}
        >{pageNumber}</button>)
